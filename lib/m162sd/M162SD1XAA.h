...
#define VFDM162_CGRAMADW \
    0x40  // + Y2..Y0 CGRAM Address Bit, n = 0 to 2
...
M162SD13AA::M162SD13AA() {
    curPos = 0;
    SPI.begin(IO_VFD_CP, -1, IO_VFD_DA, -1);
}
...
void M162SD13AA::init(uint8_t cspin) {
    ...
    digitalWrite(IO_VFD_RST, 1);
    delay(500);
    digitalWrite(IO_VFD_RST, 0);
    delay(500);
    digitalWrite(IO_VFD_RST, 1);
    delay(500);
...
void M162SD13AA::powerOn() {
    digitalWrite(IO_VFD_EN, HIGH);
}
void M162SD13AA::powerOff() {
    digitalWrite(IO_VFD_EN, LOW);
}